...
const formatDiscordMessage = async (trade) => {
	...
	const fields = [
		...
		{
			name: 'ETH Price',
			value: formatValue(trade.ethPrice, 2, 'currency'),
			inline: true,
		},
		{
			name: 'Execution ID',
			value: trade.executionId,
			inline: true,
		}
	];

	const title = `${trade.assetSymbol} ${trade.side}`;
	const description = `**${traderLabel}** ${action} **${formatValue(trade.quantity, 2)} ${trade.assetSymbol}** at **${formatValue(trade.price, 4, 'currency')}** per token`;
	...
	return {
		...
		embeds: [
			{
				author: {
					name: 'Ondo Global Market',
					icon_url: 'https://assets-global.website-files.com/62c6c96bb93c560764c906e5/62c6c96bb93c56d6a2c907fc_ondo-logotype-dark.svg'
				},
				title: title,
				...
				fields,
				footer: {
					text: `Chain ID: ${trade.chainId} | User ID: ${trade.userId.slice(0, 10)}...`
				},
...
const formatTwitterMessage = async (trade) => {
	...
	// Calculate value in ETH for reference
	const valueInEth = trade.totalValue / trade.ethPrice;
	
	let twitterMessage = `${emoji} GM Trade Alert\n\n`;
	twitterMessage += `${traderLabel} ${action} ${formatValue(trade.quantity, 2)} ${trade.assetSymbol} `;
	twitterMessage += `at ${formatValue(trade.price, 4, 'currency')} per token\n\n`;
	twitterMessage += `Total: ${formatValue(trade.totalValue, 2, 'currency')} (${formatValue(valueInEth, 3)} ETH)\n`;
